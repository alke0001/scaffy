...
- **Routes vs views vs ui/ (ADR-016):** `src/routes/**/+page.svelte` stays thin (import view only). Screen copy, example data, and wiring live in `components/<area>/`. Domain-agnostic primitives live in `components/ui/` (props in, events out — no product copy). Static logos/images in `src/lib/assets/`. If placement is unclear when adding a component, ask before creating files.
