...
const inDir = join(root, 'present/diagrams');
const outDir = join(root, 'present/assets/diagrams');
...
if (!existsSync(mmdc)) {
	console.error('mmdc not found — run: pnpm add -D @mermaid-js/mermaid-cli');
	process.exit(1);
}
...
for (const file of mmdFiles) {
	...
	console.log(`rendered ${file} → present/assets/diagrams/${file.replace(/\.mmd$/, '.svg')}`);
}
